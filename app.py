...
CHECK_INTERVAL_SECONDS = 5 # Thời gian chờ giữa các lần kiểm tra phiên mới (chỉ dùng cho polling, không dùng cho webhook chính)
...
# --- MÀU SẮC CHO CONSOLE --- (Không dùng trực tiếp cho Telegram)
...
telegram_application = None # Đổi tên để tránh xung đột với Flask app
...
def load_lich_su():
    ...
    try:
        if os.path.exists(LICHSU_FILE):
            ...
            print(f"Đã tải lịch sử cầu: {lich_su}")
        else:
            print("File lịch sử cầu không tồn tại, bắt đầu với lịch sử rỗng.")
    except IOError as e:
            ...
            with open(filepath, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        print(f"Cảnh báo: Lỗi định dạng dòng trong file '{filepath}': {line}")
                        continue
            print(f"Đã tải {len(patterns)} pattern từ '{filepath}'.")
        except IOError as e:
            print(f"Lỗi khi đọc file cầu '{filepath}': {e}")
    else:
        print(f"File pattern '{filepath}' không tồn tại, bắt đầu với pattern rỗng.")
    return patterns
...
def load_pattern_counter():
    ...
    if os.path.exists(PATTERN_COUNT_FILE):
        try:
            ...
            print(f"Đã tải bộ đếm pattern từ '{PATTERN_COUNT_FILE}'. Tổng số pattern: {len(pattern_counter)}")
        except (json.JSONDecodeError, IOError) as e:
            print(f"Cảnh báo: Không thể tải '{PATTERN_COUNT_FILE}'. Bắt đầu lại bộ đếm.")
            pattern_counter = defaultdict(lambda: {"T": 0, "X": 0})
    else:
        print("File bộ đếm pattern không tồn tại, bắt đầu với bộ đếm rỗng.")
...
def save_pattern_counter():
    ...
    try:
        ...
        print(f"Đã lưu bộ đếm pattern vào '{PATTERN_COUNT_FILE}'.")
    except IOError as e:
...
def chot_keo_cuoi_cung(predictions):
    ...
    # Nếu có sự xung đột, tìm dự đoán có độ chính xác cao nhất
...
def simulate_md5_analysis():
    ...
    # Sử dụng thông tin từ User memory
    # "Lưu ý rằng cứ 2 lần phân tích MD5 cho kết quả 'Gãy' thì sẽ có 1 lần cho kết quả khác."
            ...
            if total >= AI_LEARN_THRESHOLD_COUNT:
                ...
                if prediction_to_learn:
                    try:
                        ...
                        load_all_patterns() # Tải lại AI patterns sau khi học
...
async def send_telegram_message(context: ContextTypes.DEFAULT_TYPE, message_text: str):
    ...
    if chat_id:
        try:
            ...
            print(f"Lỗi khi gửi tin nhắn Telegram đến chat_id {chat_id}: {e}")
            # Xóa chat_id nếu có lỗi gửi để tránh lỗi lặp lại
            # global chat_id
            # chat_id = None
    else:
...
# ==== LOGIC XỬ LÝ PHIÊN MỚI ====
async def process_new_session(context: ContextTypes.DEFAULT_TYPE):
    """
    Xử lý một phiên mới từ API, thực hiện dự đoán, cập nhật lịch sử và gửi tin nhắn.
    """
    global last_processed_phien, chat_id

    # Nếu chưa có chat_id, không làm gì cả
    if not chat_id:
        print("Chưa có Chat ID, bỏ qua xử lý phiên mới.")
        return
    ...
    if last_processed_phien is None or phien_api > last_processed_phien:
        print(f"Phát hiện phiên mới: {phien_api}. Đang xử lý...")
        ...
        print(f"Phiên {phien_api} đã được xử lý hoặc chưa có phiên mới. Bỏ qua.")
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_chat:
        ...
        # Bắt đầu vòng lặp chính (polling API) thông qua job_queue
        # Đảm bảo task này chỉ được tạo một lần
        if 'api_polling_task' not in context.job_queue.jobs():
            context.job_queue.run_repeating(
                process_new_session,
                interval=CHECK_INTERVAL_SECONDS,
                first=1, # Chạy lần đầu tiên sau 1 giây
                name='api_polling_task'
            )
    else:
        print("Không thể lấy effective_chat từ lệnh /start.")
...
async def du_doan_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_chat:
        ...
        await process_new_session(context)
    else:
        print("Không thể lấy effective_chat từ lệnh /du_doan.")

async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Gửi tin nhắn trợ giúp."""
    await update.message.reply_html(
        "Các lệnh có sẵn:\n"
        "  /start - Khởi động bot và bắt đầu theo dõi các phiên.\n"
        "  /dudoan - Yêu cầu bot dự đoán ngay lập tức phiên hiện tại.\n"
        "  /help - Hiển thị các lệnh này."
    )

async def post_init(application: Application):
    """
    Thiết lập webhook sau khi ứng dụng được khởi tạo.
    """
    token = os.getenv("TELEGRAM_BOT_TOKEN")
    webhook_url = os.getenv("WEBHOOK_URL")

    if not token or not webhook_url:
        print(f"{RED}LỖI: TELEGRAM_BOT_TOKEN hoặc WEBHOOK_URL chưa được đặt. Webhook sẽ không được thiết lập.{RESET}")
        return

    print(f"Đang thiết lập webhook tới: {webhook_url}/telegram")
    await application.bot.set_webhook(url=f"{webhook_url}/telegram")
    print(f"{GREEN}Webhook đã được thiết lập thành công!{RESET}")
    ...
    print(f"======================================================================{RESET}")
    print(f"{GREEN}Bot Telegram đã sẵn sàng (Webhook mode).{RESET}")
    print(f"Kiểm tra token: {token[:5]}...{token[-5:]}")


# ==== CHẠY BOT TELEGRAM (Sử dụng Webhook) ====
async def setup_telegram_bot():
    """Hàm thiết lập và chạy bot Telegram cho webhook."""
    ...
    telegram_application = Application.builder().token(TELEGRAM_BOT_TOKEN).post_init(post_init).build()

    # Thêm các trình xử lý lệnh
    telegram_application.add_handler(CommandHandler("start", start_command))
    telegram_application.add_handler(CommandHandler("du_doan", du_doan_command))
    telegram_application.add_handler(CommandHandler("help", help_command))

    # Xử lý các tin nhắn khác nếu cần (ví dụ: in ra thông báo không hiểu lệnh)
    telegram_application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, lambda u, c: u.message.reply_text("Xin lỗi, tôi chỉ hiểu các lệnh /start, /dudoan và /help.")))
...
@app.route('/')
def home():
    """Endpoint gốc để kiểm tra trạng thái dịch vụ."""
    return "TX Pro AI Bot đang chạy!", 200

@app.route('/telegram', methods=['POST'])
async def telegram_webhook():
    """Endpoint xử lý các cập nhật từ Telegram webhook."""
    if request.method == "POST":
        update = Update.de_json(request.get_json(force=True), telegram_application.bot)
        await telegram_application.process_update(update)
        return "ok", 200
    return "ok", 200 # Trả về 200 OK cho GET requests để tránh lỗi
...
async def start_flask_app():
    ...
    # This function is not directly called by gunicorn.
    # Gunicorn will load the 'app' object directly.
    pass

if __name__ == "__main__":
    # Khởi tạo Telegram Application
    # async context for bot setup
    telegram_application = asyncio.run(setup_telegram_bot())

    # Create dummy files if they don't exist
    if not os.path.exists(LICHSU_FILE):
        open(LICHSU_FILE, 'a').close()
    if not os.path.exists(DUDOAN_FILE):
        with open(DUDOAN_FILE, 'w', encoding='utf-8') as f:
            f.write("# Đây là file chứa các pattern dự đoán VIP (AI 1).\n")
            f.write("# Định dạng: [pattern] => Dự đoán: [T/X] - Loại cầu: [Tên loại cầu]\n")
    if not os.path.exists(AI_FILE):
        with open(AI_FILE, 'w', encoding='utf-8') as f:
            f.write("# Đây là file chứa các pattern AI tự học (AI 2).\n")
            f.write("# Định dạng: [pattern] => Dự đoán: [T/X] - Loại cầu: AI Tự Học\n")
    if not os.path.exists(PATTERN_COUNT_FILE):
        with open(PATTERN_COUNT_FILE, 'w', encoding='utf-8') as f:
            json.dump({}, f)


    # Tải dữ liệu cần thiết khi khởi động tool (đã chuyển vào post_init)
    # load_pattern_counter()
    # load_lich_su()
    # load_all_patterns()
    ...
    # When deployed with Gunicorn, Gunicorn will run the app object.
    # The `main` function for `run_polling` is removed.
    # The Flask app will be served by Gunicorn.
    # You would typically run this with: gunicorn app:app -b 0.0.0.0:$PORT
    # For local development, you can still run `python app.py` if you uncomment app.run()
    # However, for Render, Gunicorn is the way to go.
    print(f"{BOLD}{GREEN}======================================================================")
    print(f"       TOOL TX - Quangdz /Trung Ngu (Phiên bản Telegram - Webhook)     ")
    print(f"======================================================================{RESET}")
    print(f"{GREEN}Ứng dụng Flask đã sẵn sàng. Gunicorn sẽ khởi chạy nó.{RESET}")
    print("Vui lòng đảm bảo các biến môi trường TELEGRAM_BOT_TOKEN và WEBHOOK_URL đã được thiết lập.")
